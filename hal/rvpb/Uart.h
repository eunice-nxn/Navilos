/*
 * Uart.h
 * Origin Source from https://github.com/navilera/Navilos
 */
...
typedef union UARTIFLS_t
{
		  uint32_t all;
		  struct {
					 uint32_t TXIFLSET:3;	// 2:0
					 uint32_t RXIFLSET:3; 	// 5:3
					 uint32_t reserved:26;	
		  } bits;
} UARTIFLS_t;


typedef union UARTMISC_t
{
		  uint32_t all;
		  struct {
					 uint32_t RIMIM:1;		// 0
